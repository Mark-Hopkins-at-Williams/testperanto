...
JSON_PATH="/mnt/storage/jcheigh/treebank-scrape/data/peranto_json"
DATA_PATH="/mnt/storage/jcheigh/treebank-scrape/data/peranto_data"
PERANTO_PATH="/mnt/storage/jcheigh/testperanto"

for json_file in $JSON_PATH/amr_*.json; do
    strength=$(echo $json_file | grep -o -E 's[0-9]+' | sed 's/s//')
    discount=$(echo $json_file | grep -o -E 'd[0-9]+' | sed 's/d//')
    
    python $PERANTO_PATH/scripts/generate.py -c $json_file $PERANTO_PATH/examples/svo/middleman1.json $PERANTO_PATH/examples/svo/english1.json --sents -n 5617 > $DATA_PATH/peranto_s${strength}_d${discount}.txt
done
