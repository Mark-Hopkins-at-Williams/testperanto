#!/bin/sh
        ...
        #SBATCH -o nn.arg0.y0.out  # File to which STDOUT will be written
        #SBATCH -e error.err        # File to which STDERR will be written
        #SBATCH --gres=gpu:0        # Request 0 GPUs

        JSON_PATH="/mnt/storage/jcheigh/testperanto/hyperparam-tuning/experiment_data/json_data/nn.arg0.y0"
        ...
        for json_file in $JSON_PATH/nn.arg0.y0_amr_*.json; do
            strength=$(echo $json_file | grep -o -E 's[0-9]+' | sed 's/s//')
            discount=$(echo $json_file | grep -o -E 'd[0-9]+' | sed 's/d//')
            
            python $PERANTO_PATH/scripts/generate.py -c $json_file $PERANTO_PATH/examples/svo/middleman1.json $PERANTO_PATH/examples/svo/english1.json --sents -n 5897 > $DATA_PATH/nn.arg0.y0/peranto_nn.arg0.y0_s${strength}_d${discount}.txt
        done
