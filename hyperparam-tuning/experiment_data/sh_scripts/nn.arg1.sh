#!/bin/sh
        ...
        for json_file in $JSON_PATH/nn.arg1_amr_*.json; do
            strength=$(echo $json_file | grep -o -E 's[0-9]+' | sed 's/s//')
            discount=$(echo $json_file | grep -o -E 'd[0-9]+' | sed 's/d//')
            
            python $PERANTO_PATH/scripts/generate.py -c $json_file $PERANTO_PATH/examples/svo/middleman1.json $PERANTO_PATH/examples/svo/english1.json --sents -n 5897 > $DATA_PATH/peranto_nn.arg1_s${strength}_d${discount}.txt
        done
