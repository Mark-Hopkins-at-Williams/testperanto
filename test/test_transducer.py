...
class TestTransducer(unittest.TestCase):
    ...
    def test_transducer(self):
        ...
        rules = [IndexedTreeTransducerRule(r) for r in rules]
        grammar = IndexedRuleSet(rules)
    ...
    def test_transducer2(self):
        rule_strs = ['($qnp (NP $x1 $x2)) -> (NP ($qnn $x2) ($qadj $x1))',
                     '($qnn $x1) -> (NN $x1)',
                     '($qadj $x1) -> (ADJ $x1)']
        grammar = IndexedRuleSet([IndexedTreeTransducerRule(r) for r in rule_strs])
        transducer = TreeTransducer(grammar)
    ...
    def test_transducer_cascade(self):
        ...
        grammar0 = IndexedRuleSet([IndexedTreeTransducerRule(r) for r in rule_strs0])
        transducer0 = TreeTransducer(grammar0)
        rule_strs1 = ['($qs (NP $x1 $x2)) -> (NP ($qnn $x2) ($qadj $x1))',
                      '($qnn $x1) -> (NN $x1)',
                      '($qadj $x1) -> (ADJ $x1)']
        grammar1 = IndexedRuleSet([IndexedTreeTransducerRule(r) for r in rule_strs1])
        transducer1 = TreeTransducer(grammar1)
        rule_strs2 = ['($qs (NP $x1 $x2)) -> (NP ($qtok $x1) ($qtok $x2))',
                      '($qtok (ADJ red)) -> (ADJ rouge)',
                      '($qtok (NN rum)) -> (NN rhum)']
        grammar2 = IndexedRuleSet([IndexedTreeTransducerRule(r) for r in rule_strs2])
