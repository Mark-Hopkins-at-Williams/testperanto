...
register_word_generator("JapaneseStems",
                        AtomBasedWordGenerator(lookup_word_generator('EnglishSyllables'),
...
class WordGeneratorInitError(Exception):
    """Thrown when the user attempts to initialize an unrecognized type of word generator."""
...
class WordGeneratorFactory(object):
    ...
    def create_generator(self, generator_name):
        if generator_name == 'EnglishSyllables':
            ...
            word_length_distribution = CategoricalDistribution([0,0,0.2,0.5,0.2,0.1])
            ...
            word_length_distribution = CategoricalDistribution([0,0.1,0.5,0.3,0.1])
            return AtomBasedWordGenerator( atom_generator, word_length_distribution )
